...
export function setupApiRoutes(app: Express) {
  ...
  app.post('/api/analyze', async (req: Request, res: Response) => {
    try {
      ...
      const vibe: VibeAnalysis = { genres: genresRecord };
   ...
   */
  app.post('/api/gif-search', async (req: Request, res: Response) => {
    try {
      const { base64Image } = req.body as { base64Image: string };
  ...
  app.post('/api/title', async (req: Request, res: Response) => {
    try {
      const { base64Image, genres } = req.body as { base64Image: string; genres: string[] };
